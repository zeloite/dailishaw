...
import { Minimize, Maximize, ArrowLeft, RotateCw } from "lucide-react";
...
export default function UserMediaPage() {
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    const getNextProductFirstImageIndex = (
      currentIndex: number,
    ): number | null => {
      ...
      if (
        ...
        if (currentCategoryIndex < categories.length - 1) {
          ...
          return nextCategoryImages.length > 0
            ? images.findIndex((img) => img.id === nextCategoryImages[0].id)
            : null;
        }
        return null;
      }
      ...
      if (nextProduct) {
        const nextProductImages = images.filter(
          (img) => img.product_id === nextProduct.id,
        );
        return nextProductImages.length > 0
          ? images.findIndex((img) => img.id === nextProductImages[0].id)
          : null;
      }
    ...
    const getPreviousProductLastImageIndex = (
      currentIndex: number,
    ): number | null => {
      if (currentIndex === null) return null;
      ...
      if (previousProduct) {
        const previousProductImages = images.filter(
          (img) => img.product_id === previousProduct.id,
        );
        return previousProductImages.length > 0
          ? images.findIndex(
              (img) =>
                img.id ===
                previousProductImages[previousProductImages.length - 1].id,
            )
          : null;
      }
      ...
      if (currentCategoryIndex > 0) {
        ...
        return prevCategoryImages.length > 0
          ? images.findIndex(
              (img) =>
                img.id === prevCategoryImages[prevCategoryImages.length - 1].id,
            )
          : null;
      }
    ...
    if (currentImageIndex !== null && selectedCategoryId) {
      ...
      if (isLeftSwipe) {
        ...
        if (nextIndex !== null) {
          ...
          const nextProductFirstImageIndex =
            getNextProductFirstImageIndex(currentImageIndex);
      ...
      if (isRightSwipe) {
        ...
        if (prevIndex !== null) {
          ...
          const previousProductLastImageIndex =
            getPreviousProductLastImageIndex(currentImageIndex);
  ...
  return (
    <>
      ...
      <div
        ref={containerRef}
        className="fixed inset-0 w-full h-full bg-black overflow-hidden touch-none"
      ...
      >
        ...
        <div className="absolute inset-0 bottom-[3rem] w-full h-[calc(100%-3rem)] overflow-hidden bg-black flex items-center justify-center">
          {loading ? (
            ...
            <div
              ...
              className="w-full h-full cursor-zoom-in relative overflow-hidden flex items-center justify-center"
        ...
        {isFullscreen ? (
          ...
          <div className="fixed bottom-0 left-0 right-0 z-30 bg-black/80 backdrop-blur-sm min-h-[3rem] h-auto flex items-center shadow-lg border-t border-white/20 px-1">
            <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 overflow-x-auto px-2 py-2 sm:px-3 sm:py-2 md:px-4 md:py-2.5 scrollbar-hide">
