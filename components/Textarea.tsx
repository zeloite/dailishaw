...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ label, icon, className = '', ...props }, ref) => {
    const baseStyles =
      'w-full bg-transparent rounded-md border border-stroke dark:border-dark-3 p-5 text-dark-6 outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-gray-2 dark:disabled:bg-dark-4 dark:disabled:border-dark-4'

    const paddingStyles = icon ? 'pl-12 p-3' : ''
    ...
    return (
      <div className="w-full">
        ...
        <div className="relative">
          ...
          />
          {icon && (
            <span className="absolute top-[18px] left-4">{icon}</span>
          )}
        </div>
      </div>
    )
